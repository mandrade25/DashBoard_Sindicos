...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  if (outroComNome) {
    return NextResponse.json(
      { error: "Já existe outro condomínio com este nome." },
      { status: 409 },
    );
  }
  ...
  await prisma.$transaction(async (tx) => {
    await tx.condominio.update({
      where: { id: params.id },
      data: { nome: data.nome, percentualRepasse: data.percentualRepasse, dataInicio: new Date(`${data.dataInicio}-01`) },
    });
    ...
    if (usuarioPorEmail) {
      await tx.usuario.update({
        ...
        data: {
          nome: data.sindicoNome,
          role: "SINDICO",
          condominioId: usuarioPorEmail.condominioId ?? params.id,
          ...(senhaHash ? { senhaHash } : {}),
        },
      });
